...
def circuit_gen(env, quantum_instance=None):
    ...
    if quantum_instance:
        qc = qiskit.transpile(
            ...
            basis_gates=quantum_instance.configuration().basis_gates,
...
def manual_qaoa(env, backend=None, hard_scale=None, optimizer='COBYLA',
    ...
    qubo = solver.construct_qubo(env, hard_scale)
    ...
    def execute(params):
        ...
        qc = circ.bind_parameters({circ.parameters[i]: params[i]
        ...
        counts = {format(x, '0' + str(qc.num_qubits) + 'b'): counts[0][x]
                  for x in counts[0]}
        fun(counts)
    ...
    opt_params = scipy.optimize.minimize(execute, initial_point, method=optimizer, options=options, jac=jac, hess = hess)
    if type(opt_params) == scipy.optimize._optimize.OptimizeResult:
        print("opt_params.x")
        circuit = circ.bind_parameters({circ.parameters[i]: opt_params.x[i]
                                   for i in range(len(opt_params.x))})
    else:
        print("opt_params")
        circuit = circ.bind_parameters({circ.parameters[i]: opt_params[i]
    ...
    counts = {format(x, '0' + str(circuit.num_qubits) + 'b'):
              result.quasi_dists[0][x] for x in result.quasi_dists[0]}
    ...
    for samp in counts:
        ret.solutions.append({circuit.qregs[i].name: x != '0'
