...
## benchmarking forked from https://github.com/ejwilson3/nchoosek_sc22/blob/main/run_problems.py, adapted to new solver/workflow
...
def run_env(env, check_env, solver, simulator, quantum_instance, num_reads, p, idx):
    qtime1 = datetime.datetime.now()
    qc, objf = nchoosek.solver.circuit_gen(env, None)
    print("qc from nck has " + str(len(qc.qubits)) + " qubits")
    ...
    print(solver)
    if solver == "qaoa" or solver == "qaoa_nono" or solver == "qaoa_hard":
        synth_solver = QAOASolver(qc, objf)
        opt_params = synth_solver.minimize(
            [1.0, 1.0], options={"maxiter": 60 * len(env.ports())}
        )
    elif solver == "synth" or solver == "synth_nono" or solver == "synth_hard":
        synth_solver = QAOASynthesisSolver(qc, objf, noise_model=noise_model)
        opt_params = synth_solver.minimize(
            [1.0, 1.0],
            {"maxiter": 6 * len(env.ports())},
            {"maxiter": 1000 * len(env.ports())},
        )
    elif solver == "inst" or solver == "inst_nono" or solver == "inst_hard":
        synth_solver = QAOAInstantiationSynthesisSolver(
            qc, objf, noise_model=noise_model
        )
        opt_params = synth_solver.minimize(
            [1.0, 1.0],
            {"maxiter": len(env.ports())},
            {"maxiter": 1000 * len(env.ports())},
        )
    else:
        print("invalid solver; input checking should avoid this")

    # print('end circuit gen')
    stime2 = datetime.datetime.now()

    opt_qc = synth_solver.get_found_circuit(opt_params)
    ...
    while p > 1:
        opt_qc.h(
            range(len(env.ports()))
        )  # undo the state prep that is included in the synthesized version.
        opt_qc = opt_qc.compose(temp)
        p = p - 1
        # opt_qc.draw(output='mpl', interactive=True, filename='circ' + str(p) + '.png')
    opt_qc.measure_all()
    # opt_qc.draw(output='mpl', interactive=True, filename='circ_final.png')

    if simulator:
        if solver == "qaoa_nono" or solver == "qaoa":
            opt_qc = qiskit.transpile(opt_qc, basis_gates=["cx", "u3"])
        if solver == "qaoa_nono" or solver == "synth_nono" or solver == "inst_nono":
            counts, _ = synth_solver.run_on_simulator(opt_qc, w_noise=False)
        else:  # simulate with noise
            counts, _ = synth_solver.run_on_simulator(opt_qc, w_noise=True)
        solutions = helpers.create_nck_solution(
            env, counts, amount=5
        )  # NOTE `amount` sets the number of most likely solutions to retrieve
        jobID = None
    else:
        transpiled = qiskit.compiler.transpile(opt_qc, backend=backend)
        jobID = backend.run(transpiled)
        solutions = None

    del synth_solver

    # res = env.solve(solver=solver) # nck solver.
    res = nckResult()
    res.tallies = None  # NOTE for Ellis, how is this used?
    res.solutions = solutions
    res.variables = [x for x in env.ports()]
    res.solver_times = (stime1, stime2)
    res.qubo_times = (qtime1, qtime2)
    res.depth = opt_qc.depth()
    res.qubit_names = [x.register.name for x in opt_qc.qubits]  # list(env.ports())
    res.qubits = opt_qc.num_qubits

    if simulator:
        res.jobID = None
    else:  # for hardware jobs, will access jobs later based on this info
        res.jobID = jobID
        line = str(idx) + "\t" + str(res.jobID) + "\t" + str(res.qubit_names) + "\n"
        with open("jobIDS_ports_" + solver + ".txt", "a") as f:
            f.write(line)

    if simulator:
        res.jobIDs = [res.jobID]
        res.solutions = (
            jobID.result().get_counts()
        )  ## TODO, seperate analysis of results once hardware jobs complete
        ...
        if (not ch or not res or not res.solutions) and ~simulator:
            res = None
            count = None
            v = None
            valq = None
        ...
        # count: the number of solutions which matched soft constraints to checkenv
        # v: the number of times the hard constraints are met
        # valq: WHICH solutions satisfy hard constraints
        # check_quality: the number of soft constraints z3 solution satisfies
        # qualitites: the number of soft constraints each found solution satisfies (sorted by probability)
        #
        ###
...
def run_graph(
    ...
    for idx, env in enumerate(envs):
        ...
        # port_order = port_order_str.replace('[', "").replace("'", "").replace(']', "").split(", ")

        res, count, v, valq, check_quality, qualities, validations = run_env(
            env, check_envs[idx], solver, simulator, quantum_instance, num_reads, p, idx
        )
        # res, count, v = run_env_2(env, job, port_order, check_envs[idx], solver, simulator, quantum_instance, num_reads, idx)
...
def run_other(
    ...
    for idx, env in enumerate(envs):
        # Add None to the lists in order to keep indices in line
        if not env or len(nchoosek.solver.circuit_gen(env, None)[0].qubits) > nqubs:
            print("invalid an env (none or too large cicruit)")
            results.append(None)
            total_counts.append(None)
            opt_counts.append(None)
            good_counts.append(None)
            ...
            check_quals.append(None)
            soft_counts.append(None)
            all_constraint_validation.append(None)
            times.append(datetime.datetime.now())
            continue
        # Run the actual problem and add it to the lists
        res, count, v, valq, check_quality, qualities, validation = run_env(
            env, check_envs[idx], solver, simulator, quantum_instance, num_reads, p, idx
        )
        print(str(idx + 1) + "/" + str(sum(x is not None for x in envs)) + " complete")
        if simulator:
            ...
            check_quals.append(check_quality)
            soft_counts.append(qualities)
            all_constraint_validation.append(validation)
            if res.jobIDs:
                n_jobs.append(len(res.jobIDs))
            else:
                n_jobs.append(0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        if (
            ...
            and solver != "qaoa_hard"
            and solver != "inst_hard"
        ):
    ...
    if len(sys.argv) < 3:
        if solver == "synth_hard" or solver == "qaoa_hard" or solver == "inst_hard":
            print("using hardware")
            simulator = False
            nqubs = backend.configuration().n_qubits
        else:
            print("hardware option not detected, defaulting to simulator")
            simulator = True
